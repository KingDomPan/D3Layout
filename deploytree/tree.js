...
(function () {
  ...
  var totalToDeploy = 23; // 待部署的结点数量
  var rows = Math.ceil(totalToDeploy / numEveryRow); // 一共需要几行

  var hasDeployNum = 0;
  for (var row = 0; row < rows; row++) {
    for (var num = 0; num < numEveryRow; num++) {
      if (hasDeployNum < totalToDeploy) {
        ...
        emitter.on("UploadWave" + (row + 1).toString() + (num + 1).toString(), q.update);
        hasDeployNum++;
      } else {
        break;
      }
  ...
  images.forEach(function (ele, index) {
    ...
    var belongTorow = Math.floor(index / numEveryRow);
    var rowIndex = index % numEveryRow;
    clone
      .animate({
        fill: "#00f",
        x: gPodBasePoint.x + rowIndex * w,
        y: gPodBasePoint.y + belongTorow * h
      }, 2000, mina.easein, function () {
        emitter.emit("UploadWave" + (belongTorow + 1).toString() + (rowIndex + 1).toString(), 50);
      });
  });
